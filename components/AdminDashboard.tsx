...
import { X, RefreshCw, Database, Edit, Save, CheckCircle, ChevronDown, Plus, Trash2, MessageSquare, Bot, User as UserIcon, Users, Shield, Check, Lock, List, Layout, FileText, Code, Eye, ArrowRight, BookOpen, HelpCircle, CreditCard, Upload, Download, ArrowLeft } from 'lucide-react';
...
const VisualEditor: React.FC<VisualEditorProps> = ({ initialValue, onChange, placeholder }) => {
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ isOpen, onClose }) => {
    ...
    const [mobileEditorOpen, setMobileEditorOpen] = useState(false); // Mobile state for editor
    ...
    const loadContent = (serviceId: string) => {
        ...
        setMobileEditorOpen(false); // Reset mobile state
        resetFormState();
    };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 bg-black/80 backdrop-blur-md">
             <div className="bg-white md:rounded-xl shadow-2xl w-full md:max-w-6xl h-full md:h-[90vh] flex flex-col overflow-hidden">
                {/* Header */}
                <div className="bg-slate-800 p-4 flex flex-col md:flex-row justify-between items-center text-white shrink-0 gap-4 md:gap-0">
                    <div className="flex items-center gap-4 w-full md:w-auto overflow-hidden">
                        <div className="flex items-center gap-2 shrink-0">
                            <Database className="text-green-400" />
                            <h2 className="font-bold text-lg hidden md:block">Admin</h2>
                        </div>
                        <div className="flex gap-1 bg-slate-700 p-1 rounded-lg overflow-x-auto scrollbar-hide w-full md:w-auto">
                            {[
                                ...
                                <button 
                                    ...
                                    className={`px-3 py-1.5 rounded-md text-xs md:text-sm font-bold transition-colors whitespace-nowrap flex-shrink-0 ${activeTab === tab.id ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                                >
                                    {tab.label}
                                </button>
                            ))}
                ...
                <div className="flex-1 overflow-hidden flex flex-col bg-slate-50">
                    ...
                    {activeTab === 'applications' && (
                         <div className="flex-1 flex overflow-hidden">
                             {/* Application List */}
                             <div className={`${selectedApp ? 'hidden md:block' : 'block'} w-full md:w-1/3 flex-1 overflow-auto p-4 md:p-6 transition-all border-r border-slate-200`}>
                                                        ...
                                                        <tr key={record.id} className={`hover:bg-slate-50 ${selectedApp?.id === record.id ? 'bg-blue-50' : ''}`}>
                                                            ...
                                                            <td className="p-4">
                                                                <button 
                                                                    ...
                                                                    className="px-3 py-1 bg-slate-800 text-white rounded text-xs hover:bg-slate-700 whitespace-nowrap"
                                                                >
                                                                    관리
                                                                </button>
                                                            </td>
                            ...
                            <div className={`${selectedApp ? 'block' : 'hidden'} w-full md:w-2/3 flex-1 bg-slate-50 overflow-y-auto md:border-l border-slate-200`}>
                                {selectedApp && (
                                    <div className="p-4 md:p-6 space-y-6">
                                        <div className="flex justify-between items-center">
                                            <div className="flex items-center gap-2">
                                                <button onClick={() => setSelectedApp(null)} className="md:hidden p-2 -ml-2 text-slate-500 hover:bg-slate-200 rounded-full">
                                                    <ArrowLeft size={20} />
                                                </button>
                                                <h3 className="text-xl font-bold text-slate-800">신청 상세 내역</h3>
                                            </div>
                                            <button onClick={() => setSelectedApp(null)} className="hidden md:block text-slate-500">
                                                <X size={24} />
                                            </button>
                                        </div>
                    ...
                    {activeTab === 'content' && editingContent && (
                        <div className="flex-1 flex overflow-hidden">
                            {/* Sidebar / Tree */}
                            {/* Hidden on mobile if editor is active */}
                            <div className={`${mobileEditorOpen ? 'hidden md:flex' : 'flex'} w-full md:w-72 bg-white border-r border-slate-200 flex-col`}>
                                ...
                                <div className="flex-1 overflow-y-auto p-2">
                                    <button 
                                        onClick={() => { setActiveSubMenuId(null); setMobileEditorOpen(true); }}
                                        className={`w-full text-left p-3 rounded-lg text-sm font-bold flex items-center gap-2 mb-2 transition-all ${activeSubMenuId === null && mobileEditorOpen ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                                    >
                                    ...
                                    <div className="space-y-1">{editingContent.subMenus.map(sub => (<div key={sub.id} onClick={() => { setActiveSubMenuId(sub.id); setMobileEditorOpen(true); }} className={`group flex justify-between items-center w-full text-left px-3 py-2 rounded-lg text-sm transition-all cursor-pointer ${activeSubMenuId === sub.id && mobileEditorOpen ? 'bg-blue-50 text-blue-700 font-bold border border-blue-100' : 'text-slate-600 hover:bg-slate-50'}`}><span className="truncate flex-1">{sub.title}</span><button onClick={(e) => handleDeleteSubMenu(sub.id, e)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-red-500 transition-opacity" title="삭제"><Trash2 size={14} /></button></div>))}</div>
                            ...
                            <div className={`${mobileEditorOpen ? 'flex' : 'hidden md:flex'} flex-1 overflow-y-auto p-4 md:p-8 bg-slate-50`}>
                                <div className="w-full max-w-4xl mx-auto space-y-6">
                                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center sticky top-0 bg-slate-50/95 backdrop-blur py-4 z-10 border-b border-slate-200 mb-6 gap-4">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <button onClick={() => setMobileEditorOpen(false)} className="md:hidden p-1 -ml-1 text-slate-500 hover:bg-slate-200 rounded-full">
                                                    <ArrowLeft size={18} />
                                                </button>
                                                <div className="flex items-center gap-2 text-sm text-slate-500">
                                                    <span>{services.find(s => s.id === selectedServiceId)?.label}</span>
                                                    <span className="text-slate-300">/</span>
                                                    <span className="font-bold text-blue-600 truncate max-w-[150px]">{activeSubMenuId ? editingContent.subMenus.find(s => s.id === activeSubMenuId)?.title : '메인 페이지 내용'}</span>
                                                </div>
                                            </div>
                                            <h3 className="text-xl md:text-2xl font-bold text-slate-800">내용 편집</h3>
                                        </div>
                                        <button onClick={saveContent} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/30 w-full md:w-auto justify-center">
                                            {saveStatus === 'Saved!' ? <CheckCircle size={18} /> : <Save size={18} />}
                                            {saveStatus || '저장하기'}
                                        </button>
                                    </div>
                                    {activeScope ? (
                                        <div className="bg-white p-4 md:p-8 rounded-2xl shadow-sm border border-slate-200 space-y-8 animate-in fade-in slide-in-from-bottom-2">
                    ...
                    {/* ... (Other Tabs like pages, news, chat, users, admins remain preserved but ensure container is responsive) ... */}
                    {activeTab === 'pages' && (
